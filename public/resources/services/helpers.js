...
app.service('helpers', function () {
    ...
    pub.buildProductPagePath = function (product) {
        let path;
        path = "/catalog/product/" + product.store_type_api_name + "/" + _.toLower(pub.clearProductUrl(_.trim(_.toLower(_.trim(product.brand) + " " + _.trim(product.name))).replace(/ /g, "-"))) + "/" + product.product_id;
        return path;
    };
